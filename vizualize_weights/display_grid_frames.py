...
def get_frames_path(index_of_sample, data_pid, extracted_frames_path):
    ...
    rep = data_pid[index_of_sample][3]
    pid = file_name.split("_")[0]
    exercise_type = file_name.split("_")[-1]

    start_frame = data_pid[index_of_sample][1]
    end_frame = data_pid[index_of_sample][2]

    frames_path = os.path.join(extracted_frames_path, pid, exercise_type, rep)
    ...
    if not os.path.exists(frames_path):
        logger.info("Frames path does not exist: {}".format(frames_path))
    return frames_path, start_frame, end_frame
...
def plot_grid(list_of_images, list_of_titles, actual_label, predicted_label, predicted_prob, file_name, output_path,
              figsize=(15, 15)):
...
def plot_frames(top_idx, top_values, frames_path, actual_label, predicted_label, predicted_prob, file_name, start_frame,
                end_frame, output_path, figsize=(15, 15)):
    fig = plt.figure(figsize=figsize)
    n_cols = 4
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            if i * n_cols + j >= len(top_idx):
                continue
            ax1 = fig.add_subplot(n_rows, n_cols, i * n_cols + j + 1)
            frame_number = top_idx[i * n_cols + j] + int(start_frame)
            print(frame_number)
            frame_path = os.path.join(frames_path, "frame_{}.jpg".format(frame_number))
            image = imread(frame_path)
            ax1.imshow(image)
            ax1.axis("off")
            ax1.set_title(
                "Index: " + str(top_idx[i * n_cols + j]) + " Weight: " + str(round(top_values[i * n_cols + j], 2)))
    st = plt.suptitle(
        'True label:' + actual_label + ', Predicted label:' + predicted_label + ',  Likelihood of ' + predicted_label + ': ' + str(
            predicted_prob), fontsize=30)
    st.set_y(0.95)
